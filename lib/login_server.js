/*
This file contains the LoginServer class which handles user login and signup.

Parameters:
- DROP_KICK_TIME: The time in seconds before dropping an idle user. Default is 30 seconds.
- io: The Socket.IO server instance.
- db: The Redis database connection.
- timer: The timer for ticking the server.
- sockets: A map of socket IDs to socket instances.
- sockets_count: The total number of connected sockets. 
- gamers: A map of user IDs to Gamer instances.
- gamers_count: The total number of logged in users.
- is_running: Whether the server is currently running.

Functionality:
- startInstance(): Starts a new instance of the login server. Generates a new instance ID, saves server info to Redis, initializes the Socket.IO and Express servers, starts the timer, and subscribes to Redis pub/sub channels.
- startup(): Starts the login server. Increments the server sequence to get a new instance ID and calls startInstance().
- shutdown(): Shuts down the login server. Clears the timer, deletes server info from Redis, closes the Socket.IO and Express servers, logs out all users, and unsubscribes from Redis pub/sub channels.
- tick(): Called every second by the timer. Updates the server info in Redis and checks for idle users to drop.
- onMessage(): Handles messages received on the 'server:#{id}' Redis pub/sub channel.
- onConnected(): Called when a new socket connects. Sends a hello message, listens for RPC requests and the 'disconnect' event.
- onDisconnected(): Called when a socket disconnects. Logs out any users on that socket and deletes the socket from the sockets map.
- signupUser(): Called when a 'signup' RPC request is received. Checks if a user with the requested ID already exists. If not, creates a new Gamer instance for that user ID.
*/
